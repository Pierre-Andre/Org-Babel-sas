...
(defvar sas-cli-file-path "/usr/local/bin/sas_u8"
  "Path to the program used by `run-sasbis'")
...
**** via comint avec log+output mélangé: =run-sasbis=
#+begin_src emacs-lisp :results none
(defun run-sasbis ()
...
*** Idea
The main idea of sasbis.el is to provide a way to send SAS program from emacs
...
SASBIS buffer (useful for org babel src_block and =:session= argument) and to have
...
#+end_src
*** demarrage d'un buffer *SAS* en inferior-mode
**** fonctions
#+begin_src emacs-lisp :tangle sasbis.el
...
#+end_src
Nom du buffer
#+begin_src elisp :tangle sasbis.el
...
Appel de make-comint-in-buffer-std via =sasbis-shell-make-comint=
#+begin_src emacs-lisp :tangle sasbis.el
